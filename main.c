...
void retornaProcessosIOs(FILA* IOs, FILA* altaPrioridade, int tempo) {
    ...
    for (int i = 0; i < tam; i++) {
        ...
        if (p.tempoDeRetornoIO == tempo) {
            ...
            printf(MAGENTA"[T:%03d] DESBLOQ    | P%d retornou do I/O\n"RESET, tempo, p.PID);
        }
        else {
...
void finalizarProcesso(PCB* processo, int tempo) {
    printf(VERDE"[T:%03d] FINALIZ    | P%d terminou execução\n"RESET, tempo, processo->PID);
    free(processo);
}
...
void bloquearProcesso(PCB* processo, FILA* IOS, int tempo) {
    ...
    printf(VERMELHO"[T:%03d] BLOQUEIO   | P%d iniciou I/O (retorna em T:%03d)\n"RESET, 
           tempo, processo->PID, processo->tempoDeRetornoIO);
}
...
int main(int c, char** argv) {
    ...
    printf("\n");
    printf("═══════════════════════════════════════════════════════════════\n");
    printf("                    SIMULAÇÃO DO ESCALONADOR                   \n");
    printf("═══════════════════════════════════════════════════════════════\n");
    printf("TEMPO  | EVENTO     | DESCRIÇÃO\n");
    printf("────────────────────────────────────────────────────────────────\n");
